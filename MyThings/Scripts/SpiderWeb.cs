...
public class SpiderWeb : MonoBehaviour
{
    ...
    InputAction LeftGrapple;
    private float LeftGripInput;
    
    InputAction LeftReel;
    ...
    private void Start()
    {
        ...
        LeftGrapple = gameplayActionMap.FindAction("ThrowLeftGrapple");
        LeftGrapple.performed += OnLeftGrappleInput;
        LeftGrapple.canceled += OnLeftGrappleInput;
        LeftGrapple.Enable();

        LeftReel = gameplayActionMap.FindAction("ReelLeftGrapple");
        LeftReel.performed += OnLeftReelInput;
        LeftReel.canceled += OnLeftReelInput;
        LeftReel.Enable();
    ...
    private void OnLeftGrappleInput(InputAction.CallbackContext context)
    {
        LeftGripInput = context.ReadValue<float>();
    }

    private void OnLeftReelInput(InputAction.CallbackContext context)
    {
    ...
    {
        // throwing out web
        if (LeftGripInput > 0 && currentlyInMotion == 0)
        {
            StartGrapple();
        }
        else if (LeftGripInput == 0)
        { 
            StopGrapple();
        }
        if (LeftTriggerInput > 0 && hitObject == 1 && Boosto == 1)
        {
            Boosto = 0;
